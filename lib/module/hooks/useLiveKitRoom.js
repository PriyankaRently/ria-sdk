...
export const useLiveKitRoom = ({
  ...
  const {} = state;
  const [rawDataFromAI, setRawDataFromAI] = useState(null);
  const [participantCount, setParticipantCount] = useState(0);
  const isConnected = room?.state === ConnectionState.Connected;

  // Timeout refs for connection management
  const timeoutFor10SecondsRef = useRef(null);
  const timeoutFor20SecondsRef = useRef(null);
  const timeoutFor30SecondsRef = useRef(null);
  ...
  const handleDataReceived = (payload, participant, _kind, topic) => {
    ...
    onAnalyticsEvent?.('livekit_data_received', {
      topic,
  ...
  const transcriptionHandler = (segments, participant, _publication) => {
  ...
  const attributesChangeHandler = (changedAttributes, _participant) => {
...
//# sourceMappingURL=useLiveKitRoom.js.map