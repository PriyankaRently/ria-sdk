...
export const useChatMessages = () => {
  ...
  const { state, storeChatMessage } = useRiaChatBot();
  const { previousChatHistory, chatMessages } = state;
    ...
    if (userChatMessages.length > 0) {
      ...
      if (latestChatMessage && !chatMessages.some(msg => msg.id === latestChatMessage.id)) {
        const newMessage: TChatMessageType = {
          ...
          timestamp: new Date(latestChatMessage.timestamp).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }),
          likeStatus: 0,
        };
  ...
  useEffect(() => {
    if (previousChatHistory && previousChatHistory.length > 0) {
      const formattedMessages: TChatMessageType[] = previousChatHistory.map(item => ({
