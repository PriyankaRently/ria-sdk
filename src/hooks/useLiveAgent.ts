...
export const useLiveAgent = ({
  ...
  const { state, setLiveAgentToAIHandoff, setReconnectToRoom } = useRiaChatBot();
  const {
    liveAgentHandoffDetails,
  } = state;

  const connectedToLiveAgentRef = useRef(false);
  const liveAgentTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const chatwootIntegrationRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    chatwootIntegration.connectToChatwoot(
      liveAgentHandoffDetails,
      onMessageHandler,
      onConversationResolvedHandler
    );
